...
def chordalsolver_feas(A, b, primalstart=None, dualstart=None,
    ...
    if type(ALPHA) is not float or (ALPHA >= 0.5 or ALPHA <= 0.0):
        raise TypeError("options['alpha'] must be a float" "in the interal (0.0,0.5)")

    BETA = options["beta"]
    if type(BETA) is not float or (BETA >= 1.0 or BETA <= 0.0):
        raise TypeError("options['beta'] must be a float" "in the interal (0.0,1.0)")
    ...
    if type(DELTA) is not float or (DELTA <= 0.0 or DELTA >= 1.0):
        raise TypeError("options['delta'] must be a float" "in the interal (0.0,1.0)")
    ...
    if type(MAXITERS) is not int:
        raise TypeError("options['maxiters'] must be a positive " "integer")
    elif MAXITERS < 1:
    ...
    if (type(REFINEMENT)) is not int:
        raise TypeError("options['refinement'] must be a nonnegative " "integer")
    elif REFINEMENT < 0:
    ...
    Va = tril(spmatrix(0.0, Ia, Ja, (n, n)))
    del Ia, Ja, LIa
    ...
    if kktsolver == "chol":
        ...
        if Ns < m:
            L = list(
                zip(
                    A.CCS[0][pm[: m - Ns] + 2] - A.CCS[0][pm[: m - Ns] + 1],
                    pm[: m - Ns],
                )
            )
            L.sort(reverse=True)
        ...
        if Ns > 0:
            L = list(
                zip(
                    A.CCS[0][pm[m-Ns:m] + 2] - A.CCS[0][pm[m-Ns:m] + 1],
                    pm[m-Ns:m],
                )
            )
    ...
    if CHOLMOD:  # hack for using cholmod symbolic
        ORDER += "+CHOLMOD"
        Ve = Va + spmatrix(
            [float(i + 1) for i in range(n)], list(range(n)), list(range(n)), (n, n)
        )
    ...
    if len(A) < 0.005 * len(Va) * m or n > 15000:  # do not unpack A
        ...
        for j in pm:
            ...
            Ltmp = list(
                zip(
                    [int(k) for k in Ve[A.CCS[1][j1:j2]]],
    ...
    if kktsolver == "chol":
        ...
        for j in range(m - Ns, m):
            k = []
            for ll in Av.CCS[1][Av.CCS[0][j]:Av.CCS[0][j+1]]:
                k.append(Ip[ll])
                k.append(Jp[ll])
    ...
    def kkt_chol(L, Y):
        ...
        for j in range(0, Ns):
            V = matrix(spmatrix(1.0, Kl[j], range(len(Kl[j])), (n, len(Kl[j]))))
            trsm(L, V)
            trsm(L, V, trans="T")
            # compute column j of H (lower triangle only)
            kkl = matrix(0, (n, 1))
            for k1, k2 in enumerate(Kl[j]):
                kkl[k2] = k1
    ...
    def print_head():
        ...
        print(
            " it  pcost       dcost      gap     pres    dres    ntdecr  Omega   pstep dstep"
        )
    ...
    if primalstart is None and dualstart is None:
        ...
        Xt = cspmatrix(symb) + spmatrix(1.0, list(range(n)), list(range(n)))
        Lt = Xt.copy()
        completion(Lt)
        fI = kktsolver(Lt, Xt)
        ...
        try:
            ...
            trA = matrix(0.0, (m, 1))
            base.gemv(Av[Id, :], matrix(1.0, (n, 1)), trA, trans="T")
        ...
        try:
            ...
            nu, y = fI(
                cspmatrix(symb) + spmatrix(-1.0, list(range(n)), list(range(n))),
                matrix(0.0, (m, 1)),
            )
            Sh = Aadj(-y)
            try:
    ...
    if y is None and X is None:
        raise ValueError(
            "could not find a feasible starting point"
    ...
    for iter in range(1, MAXITERS + 2):
        ...
        if stype == "c":
            if dcost is None:
                print(
                    "%3i % .4e %-11s %.1e %.1e %7s %.1e %7s %4.2f"
                    % (iter, pcost, " ", ggap, pres, " ", ntdecr, " ", gam)
                )
            elif pcost is None:
                print(
                    "%3i %-11s % .4e %.1e %7s %.1e %.1e %13s %4.2f"
                    % (iter, " ", dcost, ggap, " ", dres, ntdecr, " ", gam)
                )
            elif scaling == "primal":
                print(
                    "%3i % .4e % .4e %.1e %.1e %.1e %.1e %7s %4.2f"
                    % (iter, pcost, dcost, ggap, pres, dres, ntdecr, " ", gam)
                )
            else:
                print(
                    "%3i % .4e % .4e %.1e %.1e %.1e %.1e %13s %4.2f"
                    % (iter, pcost, dcost, ggap, pres, dres, ntdecr, " ", gam)
                )
        elif stype == "a":
            print(
                "%3i % .4e % .4e %.1e %.1e %.1e %.1e %.1e %4.2f  %4.2f"
...
def chordalsolver_esd(
    A, b, primalstart=None, dualstart=None, scaling="primal", kktsolver="chol", p=None
):
    """
    Chordal SDP solver:
    ...
    from cvxopt import matrix, spmatrix, sqrt, blas, lapack, base, cholmod
    ...
    if primalstart is not None:
        ...
        X = cspmatrix(symb) + spmatrix(1.0, range(n), range(n))
    if dualstart is not None:
        y = dualstart["y"][pm]
        S = cspmatrix(symb) + tril(perm(symmetrize(tril(dualstart["s"])), p))
    else:
        S = cspmatrix(symb) + spmatrix(1.0, range(n), range(n))
        y = matrix(0.0, (m, 1))
    ...
    if False:
        ...
        if primalstart is None or dualstart is None:
            ...
            cholmod.numeric(L, F)
            Ic = cspmatrix(symb) + spmatrix(1.0, list(range(n)), list(range(n)))
    ...
    if scaling == "primal":
        ...
        def bres(sigma, dz=None):
            ...
            if dz:
                ...
                rbs -= (1.0 / t) * u
                rbk -= dz[4] + 1.0 / (t * tau**2) * dz[2]  # update rbk
        ...
        def bres(sigma, dz=None):
            ...
            if dz:
                ...
                hessian(L, Y, [u], inv=False, adj=None)
                rbs -= (1.0 / t) * u
                rbk -= dz[2] + 1.0 / (t * kappa**2) * dz[4]  # update rbk
    ...
    if DEBUG:
        ...
        def newton_res(sigma):
            ...
            print(
                "\033[1;32m Newton:   % .2e % .2e % .2e % .2e % .2e \033[0m"
                % (r1, r2, r3, r4, r5)
            )
    ...
    def print_head():
        print(
            "%-20s Extended self-dual embedding, %s scaling (%s)"
            % (__version__, scaling, SolvStr)
        )
        print(
            "----------------------------------------------------------------------------"
        )
    ...
    def print_exit_info():
        ...
        if DIMACS:
            print(
                "   DIMACS:  %.2e %.2e %.2e %.2e %.2e %.2e"
                % tuple([v for v in DIMACS_err])
            )
    ...
    for iter in range(MAXITERS + 1):
        ...
        if (
            (dres <= FEASTOL)
            and (pres <= FEASTOL)
            and (gap <= ABSTOL or (relgap is not None and relgap <= RELTOL))
        ):
            # Optimal
            if show_progress:
                print("Optimal solution found.")
        ...
        dy, dX, dtau, dS, dkappa = newton(0.0)
        if DEBUG:
            newton_res(0.0)
        # Compute step length
        ...
        if not step:
            if show_progress:
                print("\033[1;31mTerminated " "(small step size detected).\033[0m")
            status = "unknown"
            break
        ...
        if DEBUG:
            print(
                "\033[1;31m mu=%.2e  tau=%.2e  kappa=%.2e  sigma=%.2e \033[0m"
                % (1.0 / t, tau, kappa, sigma)
            )
...
def conelp(c, G, h, dims=None, kktsolver="chol"):
    ...
    P._blockstruct = []
    if Nl:
        P._blockstruct.append(-Nl)
    for i in Nq:
        P._blockstruct.append(i)
    for i in Ns:
        P._blockstruct.append(i)
    ...
    for k in range(P._m + 1):
        ...
        if Nl:
            u = v[:Nl]
            Il = u.I
            B.append(spmatrix(u.V, Il, Il, (Nl, Nl)))
            ptr += Nl
